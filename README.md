...
El backend soporta un modulo PRM para partners externos, oportunidades de canal, control de conflictos y repositorio de materiales.
...
- registro de partners por tenant
- clasificacion por tipo `distributor`, `reseller`, `ally`
- oportunidades asociadas a partner y cliente
- control de conflicto configurable `global` o `partner`
- repositorio de materiales comerciales y de capacitacion
- asignacion de recursos por partner

## Inteligencia Competitiva

El backend soporta un modulo de inteligencia competitiva para capturar competidores, battlecards y motivos de perdida.

Rutas principales:

- `GET /api/competitive-intelligence/competitors`
- `POST /api/competitive-intelligence/competitors`
- `PUT /api/competitive-intelligence/competitors/{uid}`
- `DELETE /api/competitive-intelligence/competitors/{uid}`
- `GET /api/competitive-intelligence/battlecards`
- `GET /api/competitive-intelligence/competitors/{uid}/battlecards`
- `POST /api/competitive-intelligence/battlecards`
- `PUT /api/competitive-intelligence/battlecards/{uid}`
- `DELETE /api/competitive-intelligence/battlecards/{uid}`
- `GET /api/competitive-intelligence/lost-reasons`
- `POST /api/competitive-intelligence/lost-reasons`
- `PUT /api/competitive-intelligence/lost-reasons/{uid}`
- `DELETE /api/competitive-intelligence/lost-reasons/{uid}`
- `GET /api/competitive-intelligence/lost-reasons/report`

Capacidades:

- catalogo de competidores por tenant
- battlecards asociadas a competidores
- motivos de perdida configurables
- reporte para detectar patrones de derrotas comerciales

## Oportunidades y Pipeline

El pipeline comercial soporta etapas configurables, tablero y resumen ejecutivo.

Rutas principales:

- `GET /api/opportunities/stages`
- `POST /api/opportunities/stages`
- `PUT /api/opportunities/stages/{uid}`
- `DELETE /api/opportunities/stages/{uid}`
- `GET /api/opportunities/board`
- `GET /api/opportunities/summary`
- `GET /api/opportunities`
- `POST /api/opportunities`
- `PUT /api/opportunities/{uid}`
- `DELETE /api/opportunities/{uid}`
...
## Costos, Rentabilidad y Cuentas por Pagar

Ademas del CRUD base de gastos, el backend soporta centros de costo, rentabilidad y cuentas por pagar operativas.

Rutas principales:

- `GET /api/expenses/cost-centers`
- `POST /api/expenses/cost-centers`
- `PUT /api/expenses/cost-centers/{uid}`
- `DELETE /api/expenses/cost-centers/{uid}`
- `GET /api/expenses/profitability`
- `GET /api/purchases/payables`
- `POST /api/purchases/orders/{uid}/receive-partial`
- `POST /api/purchases/orders/{uid}/payments`

Capacidades:

- centros de costo simples por tenant
- vista de rentabilidad operativa
- recepcion parcial de compras
- pagos a ordenes de compra y consulta de cuentas por pagar

## Operacion, Cache y Despliegue

### Paginacion y rate limit

Los endpoints indexados pueden respetar estas variables:

- `API_FORCE_INDEX_PAGINATION`
- `API_DEFAULT_PER_PAGE`
- `API_MAX_PER_PAGE`
- `API_RATE_LIMIT_PER_MINUTE`
- `API_GUEST_RATE_LIMIT_PER_MINUTE`

Notas:

- los listados mas pesados ya soportan paginacion controlada
- la respuesta paginada agrega `meta.pagination`
- el middleware `throttle:api` protege todos los endpoints API

### Cache, colas y dashboard

Para produccion, la recomendacion actual es usar Redis:
...
El logger del sistema se dejo en modo tolerante a fallos:

- si falla el log a archivo, la request no debe romperse
- para contenedores la recomendacion es `LOG_CHANNEL=stderr`
- nivel recomendado en produccion: `LOG_LEVEL=info`

### Documentos y disco dedicado

La gestion documental ya no depende ciegamente del disco general.
...
- los documentos usan disco dedicado configurable
